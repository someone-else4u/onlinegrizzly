    ...
    <section id="intel" className="relative py-24 bg-gradient-hero overflow-hidden">
      ...
      <div className="relative container mx-auto px-4">
        <div className="max-w-2xl mb-14">
          ...
          <h2 className="mt-4 font-tech text-3xl md:text-5xl font-extrabold uppercase text-primary-foreground leading-[1.1]">
            Know your rank
        ...
        <div className="grid lg:grid-cols-5 gap-4">
          ...
          <div className="lg:col-span-3 neo-panel-dark neon-border p-6 md:p-8">
            <div className="flex items-center gap-2 mb-6">
              ...
              <span className="tracking-widest-xs text-primary-foreground/70">AI Rank Predictor</span>
            </div>
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-sm border transition-all ${
                    exam === key
                      ...
                      : "border-primary/25 text-primary-foreground/70 hover:border-primary/50 hover:text-primary-foreground"
                  }`}
                >
                  {EXAMS[key].label}
                </button>
            ...
            <div className="grid sm:grid-cols-[1fr_auto] gap-6 items-end">
              <div>
                <label className="tracking-widest-xs text-primary-foreground/50">Expected score (out of {config.max})</label>
                <Slider
                  className="mt-5"
              ...
              <Input
                ...
                className="w-28 bg-primary/10 border-primary/30 text-primary-foreground font-tech text-lg"
              />
            </div>

            <div className="grid sm:grid-cols-3 gap-4 mt-8">
              <div className="rounded-xl border border-primary/20 bg-primary/10 p-5">
                <div className="tracking-widest-xs text-primary-foreground/50">Percentile</div>
                <div className="mt-2 font-tech text-3xl font-bold text-accent">{result.percentile.toFixed(2)}</div>
              </div>
              <div className="rounded-xl border border-primary/20 bg-primary/10 p-5">
                <div className="tracking-widest-xs text-primary-foreground/50">Projected AIR</div>
                <div className="mt-2 font-tech text-3xl font-bold text-primary-foreground">
                  {result.rank.toLocaleString()}
                </div>
              </div>
              <div className="rounded-xl border border-primary/20 bg-primary/10 p-5">
                <div className="tracking-widest-xs text-primary-foreground/50">Candidate pool</div>
                <div className="mt-2 font-tech text-3xl font-bold text-primary-foreground">
          ...
          <div className="lg:col-span-2 neo-panel-dark p-6 md:p-8">
            <div className="flex items-center gap-2 mb-6">
              ...
              <span className="tracking-widest-xs text-primary-foreground/70">Live Scoreboard</span>
            </div>
            ...
            {loading ? (
                ...
                {rows.map((row, i) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-primary-foreground truncate">{row.name}</p>
                      <p className="text-xs text-primary-foreground/50">{row.accuracy}% accuracy</p>
                    </div>
                    <span className="font-tech text-sm text-primary-foreground">
                      {row.score}
                      <span className="text-primary-foreground/40">/{row.totalMarks}</span>
                    </span>
              ...
              <div className="rounded-xl border border-primary/15 bg-primary/5 p-6 text-center">
                ...
                <Link to="/login">
                  <Button variant="outline" size="sm" className="mt-4 rounded-full border-primary/40 bg-transparent text-primary-foreground hover:bg-primary/10">
                    Sign in to view
                  </Button>
                </Link>
              </div>
